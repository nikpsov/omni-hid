...
> **Готовое приложение**: Ознакомьтесь с [**OmniHID Taskbar Battery Indicator**](https://github.com/nikpsov/omni-hid-taskbar-battery-indicator) — индикатором заряда батареи в панели задач Windows, созданным на базе этого движка.
...
Все команды можно запускать как по ключевому слову, так и по номеру в интерактивном меню (`0`..`9`):
...
| :-: | :--- | :--- | :--- |
| `[1]` | `scan` | `omni-hid scan [фильтр] [--all] [-r]` | Опрос периферии: заряд, статус зарядки, напряжение и протокол (стандартный режим, все устройства). |
...
| `[9]` | `registered` | `omni-hid registered [фильтр]` | Опрос **только протестированных** устройств с декларативными `.json` профилями (без эвристик). |
| `[U]` | `update` | `omni-hid update` | Синхронизация каталога профилей по воздуху (OTA) напрямую из репозитория GitHub. |
...
> **Горячая перезагрузка (Hot Reload) и OTA-обновление**: Профили в `%APPDATA%\OmniHid\devices\` и `./devices/` разделены на подпапки `verified/` (проверенные) и `unverified/` (экспериментальные/пользовательские). Они отслеживаются через `FileSystemWatcher` и перезагружаются «на лету» без перезапуска программы. Команда `omni-hid update` позволяет обновить весь каталог устройств по воздуху (OTA) с GitHub без пересборки.
...
- ⚙️ [**Справочник по CLI**](docs/cli-reference.md) — Полное руководство по всем 10 командам и диагностическим утилитам `omni-hid`.
- 📄 [**Профили устройств и Hot Reload**](docs/device-profiles.md) — Схема JSON-профилей, группы verified/unverified, папки автозагрузки и OTA-обновление.
...
## Экосистема и проекты на базе OmniHID

- 🔋 [**OmniHID Taskbar Battery Indicator**](https://github.com/nikpsov/omni-hid-taskbar-battery-indicator) — Легковесный индикатор заряда батареи беспроводной игровой периферии для панели задач Windows (системного трея), работающий на базе движка `OmniHid.Core`.
...
```
omni-hid/
├── .github/workflows/       # CI/CD: верификация сборки, релизы, OTA каталога устройств
│   └── catalog.yml          # Автосборка devices.zip при коммитах в main
├── devices/                 # Декларативные JSON-профили (файловый / OTA каталог)
│   ├── verified/            # Проверенные и протестированные профили репозитория
│   └── unverified/          # Экспериментальные, пользовательские и комьюнити-профили
